...
const firebaseConfig = {
  ...
  appId: rawAppId?.trim(),
  measurementId: rawMeasurementId?.trim(),
};

console.log(
  "Firebase Config object constructed in src/lib/firebase.ts (with .trim()):", firebaseConfig
);
...
if (
  ...
  firebaseConfig.apiKey === "YOUR_API_KEY_HERE" || // Specific placeholder check
  !firebaseConfig.projectId ||
  firebaseConfig.projectId.trim() === "" ||
  firebaseConfig.projectId.includes("YOUR_PROJECT_ID") || // General placeholder check
  firebaseConfig.projectId === "YOUR_PROJECT_ID_HERE" // Specific placeholder check
) {
  const errorMessage =
...
console.log(`[${new Date().toISOString()}] PRE-APP-INIT: Attempting Firebase App Initialization with Project ID from firebaseConfig: ${firebaseConfig.projectId}`);
// Initialize Firebase App
try {
  if (getApps().length === 0) {
    app = initializeApp(firebaseConfig);
    console.log(`[${new Date().toISOString()}] POST-APP-INIT: Firebase App Initialized successfully. App Name: ${app.name}, App Project ID from app.options: ${app.options.projectId}`);
  } else {
    app = getApp();
    console.log(`[${new Date().toISOString()}] POST-APP-INIT: Existing Firebase App retrieved. App Name: ${app.name}, App Project ID from app.options: ${app.options.projectId}`);
  }
} catch (error) {
  ...
  console.error("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
  throw new Error(initErrorMessage); // Re-throw to stop execution if core app init fails
}
...
try {
  ...
  console.error("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
  throw new Error(authErrorMessage); // Re-throw to stop execution if auth init fails
}
...
try {
  ...
  console.error("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
  throw new Error(firestoreErrorMessage); // Re-throw to stop execution if firestore init fails
}
